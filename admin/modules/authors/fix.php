...
$data = [
    'nameTitle' => 'Fix author'
];

if(!empty($_GET['id'])){
    $authorId = $_GET['id'];
    $detailAuthor = getFirstRow("SELECT * FROM author WHERE id='$authorId'");
    if(!empty($detailAuthor)){
...
if(is_Post()){
    ...
    if(empty($erorrs)){
        ...
        $statuUpdate = update('author', $dataUpdate, "id='$authorId'");

        if($statuUpdate){
            setFlashData('msg', 'Fix author success !!!');
    ...
    redirect("?module=authors&active=fix&id=$authorId#add");
